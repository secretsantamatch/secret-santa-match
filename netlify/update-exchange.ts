...
export async function handler(event: any, context: any) {
    ...
    try {
        ...
        const clientData: Partial<ExchangeData> = JSON.parse(event.body);

        if (!clientData.id) {
            return { statusCode: 400, body: JSON.stringify({ error: 'Exchange ID is required for an update.' }) };
        }
        const exchangeId = clientData.id;
        ...
        const cleanData = removeUndefined(updatePayload);
        ...
        return {
            statusCode: 500,
            body: JSON.stringify({ error: 'Failed to update the gift exchange.' }),
        };
    }
}
