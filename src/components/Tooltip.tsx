...
// FIX: Add `children` to TooltipProps to allow wrapping components.
...
const Tooltip: React.FC<TooltipProps> = ({ text, children }) => {
  return (
    // FIX: Render children if provided, otherwise default to the info icon.
    // Conditionally apply `cursor-help` only for the info icon version.
    <div className={`relative flex items-center group ${!children ? 'cursor-help' : ''}`}>
      {children || <InfoIcon />}
