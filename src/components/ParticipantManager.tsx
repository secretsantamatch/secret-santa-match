  ...
  interface ParticipantManagerProps {
    ...
    onBulkAddClick: () => void;
    // FIX: Add prop to receive the set of duplicate participant IDs
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-3">
          {participants.map((participant, index) => {
            // FIX: Check if the current participant's ID is in the duplicate set
              ...
              <div key={participant.id} className="grid grid-cols-12 gap-x-4 gap-y-2 items-start animate-fade-in-up" style={{ animationDelay: `${index * 30}ms` }}>
                <div className="col-span-12 sm:col-span-4">
                  ...
                  <input
                    ...
                    // FIX: Apply error styling if the name is a duplicate
                    className={`w-full p-2.5 border rounded-lg focus:ring-2 focus:ring-offset-1 transition ${isDuplicate ? 'border-red-500 ring-2 ring-red-200' : 'border-gray-300 focus:ring-[var(--primary-focus-ring-color)]'}`}
                  />
                  {/* FIX: Show a warning message for duplicate names */}
                  {isDuplicate && (<p className="text-red-600 text-xs mt-1">This name is a duplicate.</p>)}
                </div>
