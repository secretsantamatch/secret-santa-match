...
const GeneratorPage: React.FC = () => {
  ...
  const handleStepClick = (stepNumber: number) => {
    ...
    if (targetRef?.current) {
      ...
      // Use 'start' to ensure the top of the section is visible, with an offset.
      const offset = 20; // px
  ...
  const handleGenerateMatches = () => {
    ...
    while (attempts < maxAttempts && !generatedMatches) {
      ...
      if (possible && currentMatches.length === remainingGivers.length) generatedMatches = [...assignedMatches, ...currentMatches];
    }
    if (generatedMatches) { 
        setError(''); 
        const data: ExchangeData = {
            p: participants.filter(p => p.name.trim()),
            m: generatedMatches,
            e: exclusions,
            a: assignments,
            d: eventDetails,
        ...
        window.location.href = `/results#${encodedData}`;
    }
  ...
  return (
    <div className="bg-slate-50 min-h-screen">
      <div className="container mx-auto p-4 sm:p-6 md:p-8 max-w-5xl">
        <Header />
        {/* FIX: Pass onStepClick prop to HowItWorks component */}
        ...
        <main className="mt-8 md:mt-12 space-y-10 md:space-y-12">
          ...
          <div ref={participantsRef} className="p-6 md:p-8 bg-white rounded-2xl shadow-lg border border-gray-200">
            ...
            {/* FIX: Pass duplicateNameIds prop to ParticipantManager */}
            <ParticipantManager 
          ...
          <div ref={rulesRef} className="p-6 md:p-8 bg-white rounded-2xl shadow-lg border border-gray-200">
            ...
            <div className={`transition-all duration-500 ease-in-out overflow-hidden ${isRulesExpanded ? 'max-h-[1000px] pt-6' : 'max-h-0'}`}>
                {/* FIX: Pass exchangeDate and setExchangeDate props to Options component */}
