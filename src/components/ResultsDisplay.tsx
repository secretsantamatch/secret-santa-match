...
const ResultsPage: React.FC<ResultsPageProps> = ({ data, currentParticipantId }) => {
    ...
    useEffect(() => {
        ...
        if (!currentParticipantId) {
            ...
            const reconstructedMatches: Match[] = data.m.map(matchData => {
    ...
    const revealDateTime = useMemo(() => {
        ...
        const date = new Date(data.rd);
        if (data.rt) {
            const [hours, minutes] = data.rt.split(':');
            date.setHours(parseInt(hours, 10), parseInt(minutes, 10), 0, 0);
        } else {
            // Set to beginning of the day in UTC if no time is provided.
    ...
    useEffect(() => {
        ...
        const checkReveal = () => {
            if (new Date() >= revealDateTime) {
                ...
                if (isOrganizerView) {
                     const reconstructedMatches: Match[] = data.m.map(matchData => ({
                        giver: { ...data.p[matchData.g], id: `p${matchData.g}` },
                        receiver: { ...data.p[matchData.r], id: `p${matchData.r}` }
                    }));
                    setAllMatches(reconstructedMatches);
                }
            }
    ...
    }, [revealDateTime, isOrganizerView, data]);
    ...
    const handleCopy = (link: string, name: string, index: number) => {
    ...
    const handleCopyAll = () => {
        ...
        const allLinks = allMatches.map((match, index) => {
            const participantIndex = data.p.findIndex(p => p.name === match.giver.name);
            const link = `${baseUrl}#${hash}?id=${participantIndex}`;
            return `${match.giver.name}: ${link}`;
        }).join('\n');
        
        const message = `🎁 Secret Santa links are ready! 🎁\n\nPlease send each person their own private link below:\n\n${allLinks}`;
        navigator.clipboard.writeText(message).then(() => {
            const allCopied = allMatches.reduce((acc, _, index) => ({...acc, [index]: true}), {});
                         ...
                         <div className="p-6 md:p-8 bg-white rounded-2xl shadow-lg border border-gray-200">
                            ...
                            <div className="space-y-3 max-w-3xl mx-auto">
                               {allMatches.map((match, index) => {
                                    const participantIndex = data.p.findIndex(p => p.name === match.giver.name);
                                    const link = `${window.location.href.split('#')[0]}#${window.location.hash.substring(1).split('?')[0]}?id=${participantIndex}`;
                                    return (
                                        <div key={match.giver.id} className="flex flex-col sm:flex-row items-center justify-between p-3 bg-slate-50 rounded-lg border border-slate-200 gap-3">
                                            ...
                                            <button onClick={() => handleCopy(link, match.giver.name, index)} className={`w-full sm:w-48 text-center font-semibold py-2 px-4 rounded-md transition-colors text-white flex items-center justify-center gap-2 ${copySuccess[index] ? 'bg-green-600' : 'bg-slate-700 hover:bg-slate-800'}`}>
                            ...
                            <div className="text-center mt-6 border-t pt-6">
                                ...
                                {isRevealed ? <ResultsDisplay matches={allMatches} /> : <p className="text-slate-600">The full list of matches will be revealed here after your selected date and time.</p>}
                            </div>
                        </div>
        ...
        <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4 sm:p-6 md:p-8">
            ...
            <main className="w-full max-w-md my-8">
              <div className="bg-white p-6 sm:p-8 rounded-2xl shadow-lg border text-center">
                 ...
                 <PrintableCard 
                    match={match} eventDetails={data.e || ''} backgroundId={data.style.backgroundId} backgroundImageUrl={null}
              ...
              <div className="mt-10 bg-white p-6 sm:p-8 rounded-2xl shadow-lg border text-center">
                ...
                {isRevealed ? <ResultsDisplay matches={allMatches} /> : <CountdownTimer targetDate={data.rd!} targetTime={data.rt} />}
              </div>
            </main>
