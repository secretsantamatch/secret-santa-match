...
const ShareLinksModal: React.FC<ShareLinksModalProps> = ({ participants, getParticipantLink, onClose }) => {
  ...
  const handleCopyAllLinks = () => {
    ...
    navigator.clipboard.writeText(`Secret Santa Links:\n\n${allLinksText}`).then(() => {
          ...
          {participants.map(p => (
            <div key={p.id} className="flex items-center justify-between bg-white p-3 rounded-lg border border-slate-200">
              ...
              <button 
                ...
                className="bg-slate-800 hover:bg-slate-900 text-white font-bold py-2 px-5 text-sm rounded-lg transition-all flex items-center gap-2 w-32 justify-center"
