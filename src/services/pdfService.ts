...
import type { ExchangeData, Participant, WERules, WETheme, WEParticipant, PotluckEvent } from '../types';
...
// --- POTLUCK PDF ---
export const generatePotluckPdf = (event: PotluckEvent): void => {
    const pdf = new jsPDF('p', 'mm', 'a4') as jsPDFWithAutoTable;
    
    // Determine header color based on theme (approximate values)
    let headerColor = '#1e293b'; // Slate (Default/Corporate)
    if (event.theme === 'christmas') headerColor = '#b91c1c'; // Red
    if (event.theme === 'thanksgiving') headerColor = '#b45309'; // Amber
    if (event.theme === 'picnic') headerColor = '#047857'; // Emerald
    if (event.theme === 'fiesta') headerColor = '#be185d'; // Pink
    if (event.theme === 'classic') headerColor = '#c2410c'; // Orange

    // Title
    pdf.setFontSize(24);
    pdf.setFont('helvetica', 'bold');
    pdf.setTextColor(headerColor);
    pdf.text(event.title, pdf.internal.pageSize.getWidth() / 2, 20, { align: 'center' });
    ...
    let y = 30;
    const dateStr = new Date(event.date + 'T12:00:00').toLocaleDateString(undefined, { weekday: 'long', month: 'long', day: 'numeric' });
    pdf.text(`Hosted by: ${event.hostName}`, 15, y);
    pdf.text(`Date: ${dateStr} ${event.time ? '@ ' + event.time : ''}`, 15, y + 6);
    
    if (event.location) {
        pdf.text(`Location: ${event.location}`, 15, y + 12);
        y += 12;
    }
    if (event.dietaryNotes) {
        pdf.setTextColor(180, 83, 9); // Amber text for warnings
        pdf.setFont('helvetica', 'bold');
        pdf.text(`Note: ${event.dietaryNotes}`, 15, y + 6);
        pdf.setTextColor(50); // Reset
        pdf.setFont('helvetica', 'normal');
        y += 6;
    }
    ...
    // Build Table Body
    // Group dishes by category for better readability in PDF
    const body: any[] = [];
    
    event.categories.forEach(cat => {
        const catDishes = event.dishes.filter(d => d.categoryId === cat.id);
        
        // Add Category Header Row
        body.push([{ content: cat.name.toUpperCase(), colSpan: 3, styles: { fillColor: [241, 245, 249], fontStyle: 'bold', textColor: 50 } }]);
        
        if (catDishes.length === 0) {
            body.push(['(No dishes yet)', '', '']);
        } else {
            catDishes.forEach(dish => {
                const dietLabels = dish.dietary.map(d => d.toUpperCase()).join(', ');
                body.push([
                    dish.dishName + (dietLabels ? ` (${dietLabels})` : ''),
                    dish.guestName,
                    '' // Checkbox column for physical use
                ]);
            });
        }
    });
    ...
    pdf.autoTable({
        startY: y,
        head: [['Dish / Item', 'Bringing By', 'Arrived?']],
        body: body,
        theme: 'grid',
        headStyles: {
        ...
        columnStyles: {
            0: { cellWidth: 90 },
            1: { cellWidth: 60 },
            2: { cellWidth: 30 }, // Checkbox column
        }
    });
    ...
    for (let i = 1; i <= pageCount; i++) {
        ...
        pdf.setTextColor(150);
        pdf.text(
            `Potluck Plan generated by SecretSantaMatch.com`,
            pdf.internal.pageSize.getWidth() / 2,
    ...
    pdf.save(`${event.title.replace(/[^a-z0-9]/gi, '_')}_Potluck_List.pdf`);
};
