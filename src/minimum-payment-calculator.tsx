...
const ShareModal = ({ onClose }: { onClose: () => void }) => {
  ...
  const pinterestImageUrl = 'https://secretsantamatch.com/thumbnails/debt-calculator-promo.png'; // A descriptive image for Pinterest

  const socialLinks = [
    { name: 'Pinterest', url: `https://pinterest.com/pin/create/button/?url=${encodeURIComponent(url)}&media=${encodeURIComponent(pinterestImageUrl)}&description=${encodeURIComponent(text)}`, icon: '📌', color: 'bg-red-600 hover:bg-red-700' },
    { name: 'Facebook', url: `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(url)}`, icon: '👍', color: 'bg-blue-600 hover:bg-blue-700' },
    { name: 'Twitter', url: `https://twitter.com/intent/tweet?url=${encodeURIComponent(url)}&text=${encodeURIComponent(text)}`, icon: '🐦', color: 'bg-sky-500 hover:bg-sky-600' },
    { name: 'WhatsApp', url: `https://api.whatsapp.com/send?text=${encodeURIComponent(text)} ${encodeURIComponent(url)}`, icon: '💬', color: 'bg-green-500 hover:bg-green-600' },
  ];
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fadeIn" onClick={onClose}>
      <div className="bg-gradient-to-br from-slate-900 via-purple-900 to-slate-800 rounded-3xl max-w-lg w-full border-2 border-white/20 shadow-2xl p-8 relative" onClick={e => e.stopPropagation()}>
        <h2 className="text-3xl font-black text-white text-center mb-4">Share This Tool</h2>
        <p className="text-gray-300 text-center mb-8">Help others see the truth about minimum payments!</p>
        <div className="grid grid-cols-2 gap-4">
          {socialLinks.map(link => (
            <a href={link.url} target="_blank" rel="noopener noreferrer" key={link.name} className={`flex items-center justify-center gap-3 text-white font-bold p-4 rounded-xl transition-transform transform hover:scale-105 ${link.color}`}>
        ...
        <div className="mt-6">
          <button onClick={copyLink} className="w-full bg-white/10 hover:bg-white/20 border-2 border-white/20 text-white font-bold py-4 px-6 rounded-xl transition-colors">
...
const AdSenseBlock = () => {
    ...
    return (
        <div className="my-8">
            <div className="ad-container bg-white/5 rounded-2xl p-4 text-center text-white/50 min-h-[100px] flex items-center justify-center">
...
export default function MinimumPaymentCalculator() {
  ...
  const results = useMemo(() => {
    ...
    const scenariosWithYears = scenarios.map(s => ({
      ...s,
      debtFreeYear: s.months === Infinity ? '∞' : currentYear + Math.ceil(s.months / 12)
    }));

    // Custom payment slider result
    ...
    const customYear = customResults.months === Infinity ? '∞' : currentYear + Math.ceil(customResults.months / 12);
  ...
  const handleCalculate = () => {
    setShowResults(true);
    setCustomPayment(0); // Reset slider
    setTimeout(() => {
...
${['🔴', '🟠', '🟡', '🟢'][i]} ${s.label.toUpperCase()}
   ...
   └─────────────────────────────────────────────────────┘
   ${i > 0 ? `💰 SAVE: ${formatCurrency(results.scenarios[0].totalInterest - s.totalInterest)} vs minimum
      ...
      <div className="max-w-7xl mx-auto relative z-10">
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="text-left">
                ...
                <div className="space-y-3 text-gray-200 text-lg leading-relaxed">
                  <p>
                    <span className="font-bold text-white">Step 1:</span> Enter your credit card balance, interest rate, and what you're paying each month.
                  </p>
                  <p>
                    <span className="font-bold text-white">Step 2:</span> Click "Calculate" and we'll show you exactly when you'll be debt-free.
                  </p>
                  <p>
                    <span className="font-bold text-white">The Shocking Part:</span> If you keep paying just the minimum, you could be in debt for 10-20+ years and pay DOUBLE what you borrowed!
                  </p>
                  <p>
                    <span className="font-bold text-white">The Good News:</span> Paying even $50 more each month can save you thousands of dollars and years of your life.
                  </p>
                  <p className="text-yellow-300 font-semibold mt-4">
                    💡 Think of it this way: Would you rather pay $10,000 over 20 years, or $7,000 over 3 years? That's the difference!
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="bg-white/10 backdrop-blur-xl rounded-3xl shadow-2xl p-8 md:p-12 mb-12 border border-white/20">
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            <button
              onClick={addAccount}
              className="flex items-center gap-2 bg-blue-500 hover:bg-blue-600 text-white px-5 py-3 rounded-xl transition font-semibold text-lg"
            >
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                {accounts.length > 1 && (
                  <button
                    ...
                    className="text-red-400 hover:text-red-300 transition p-2 flex items-center gap-2"
                  >
          ...
          <div id="results-section" className="space-y-8">
              ...
              <div className="relative z-10">
                ...
                <div className="bg-white/10 backdrop-blur-xl rounded-2xl p-8 border-2 border-white/30 mb-6">
                  <div className="grid md:grid-cols-2 gap-8">
                    <div className="text-center md:text-left">
                      ...
                      <p className="text-7xl md:text-9xl font-black text-white tracking-tighter mb-3">
                        {results.scenarios[0].debtFreeYear}
                      </p>
                      <p className="text-white text-2xl font-bold">
                        That's <span className="text-yellow-300">{formatTime(results.scenarios[0].months)}</span> from now!
                      </p>
                      <p className="text-white/70 text-lg mt-2">
                        Imagine making payments for that long...
                      </p>
                    </div>
                    <div className="text-center md:text-left">
                      <p className="text-white/80 text-lg font-medium mb-2">YOU'LL PAY THIS MUCH IN TOTAL...</p>
                      <p className="text-6xl md:text-8xl font-black text-yellow-300 mb-3">
                        {formatCurrency(results.scenarios[0].totalPaid)}
                      </p>
                      <p className="text-white text-xl">
                        But you only borrowed <span className="font-bold">{formatCurrency(results.totalBalance)}</span>
                      </p>
                      <p className="text-red-200 text-2xl font-bold mt-3">
                        💸 That's {formatCurrency(results.scenarios[0].totalInterest)} burned on interest!
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white/20 backdrop-blur-xl rounded-2xl p-6 border border-white/30">
                  <p className="text-white text-xl md:text-2xl font-bold text-center">
                    💔 You'll pay <span className="text-yellow-300 text-3xl">{results.totalBalance > 0 ? ((results.scenarios[0].totalInterest / results.totalBalance) * 100).toFixed(0): 0}%</span> MORE than you borrowed!
                  </p>
                  <p className="text-white/90 text-center mt-2 text-lg">
                    That's money that could go to vacations, savings, or your kids' college fund.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-green-600 to-emerald-600 rounded-3xl p-8 md:p-12 shadow-2xl border-2 border-white/30">
              <div className="text-center mb-8">
                ...
                <p className="text-2xl text-white/95">
                  By paying just a little bit more each month, you can save THOUSANDS and get out of debt YEARS sooner.
                </p>
              </div>
                ...
                <div className="grid md:grid-cols-3 gap-6 text-center">
                  <div>
                    ...
                    <p className="text-5xl font-black text-yellow-300">
                      {formatCurrency(results.scenarios[0].totalInterest - results.scenarios[1].totalInterest)}
                    </p>
                    <p className="text-white/90 mt-2">In interest!</p>
                  </div>
                  <div>
                    <p className="text-white/80 text-lg mb-2">Debt-Free In</p>
                    <p className="text-5xl font-black text-white">
                      {results.scenarios[1].debtFreeYear}
                    </p>
                    <p className="text-white/90 mt-2">Instead of {results.scenarios[0].debtFreeYear}!</p>
                  </div>
                  <div>
                    <p className="text-white/80 text-lg mb-2">Freedom Comes</p>
                    <p className="text-4xl font-black text-white">
                      {formatTime(results.scenarios[0].months - results.scenarios[1].months)}
                    </p>
                    <p className="text-white/90 mt-2">Sooner!</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white/10 backdrop-blur-xl rounded-3xl shadow-2xl p-8 md:p-12 border border-white/20">
              <div className="text-center mb-8">
                ...
                <p className="text-xl text-gray-300">
                  Move the slider to see how paying more changes everything
                </p>
              </div>

              <div className="bg-white/5 rounded-2xl p-8 border border-white/10">
                <div className="mb-8">
                  <label className="block text-2xl font-bold text-white mb-4 text-center">
                    Add Extra Payment: <span className="text-yellow-300">{formatCurrency(customPayment)}</span> per month
                  </label>
                  <input
                    type="range"
                    min="0"
                ...
                {customPayment > 0 && (
                  <div className="bg-gradient-to-br from-blue-600 to-purple-600 rounded-2xl p-8">
                    <h3 className="text-3xl font-bold text-white mb-6 text-center">
                      If You Pay {formatCurrency(results.customPaymentTotal)}/Month:
                    </h3>
                    <div className="grid md:grid-cols-3 gap-6">
                      <div className="bg-white/20 backdrop-blur rounded-xl p-6 text-center">
                        ...
                        <p className="text-6xl font-black text-white">{results.customYear}</p>
                        <p className="text-white/90 mt-2 text-lg">{formatTime(results.customResults.months)}</p>
                      </div>
                      <div className="bg-white/20 backdrop-blur rounded-xl p-6 text-center">
                        <p className="text-white/80 text-lg mb-2">You'll Save</p>
                        <p className="text-5xl font-black text-yellow-300">
                          {formatCurrency(results.scenarios[0].totalInterest - results.customResults.totalInterest)}
                        </p>
                        <p className="text-white/90 mt-2">vs minimum payment!</p>
                      </div>
                      <div className="bg-white/20 backdrop-blur rounded-xl p-6 text-center">
                        <p className="text-white/80 text-lg mb-2">Freedom Comes</p>
                        <p className="text-5xl font-black text-white">
                          {formatTime(results.scenarios[0].months - results.customResults.months)}
                        </p>
                        <p className="text-white/90 mt-2">Sooner!</p>
                      </div>
                    </div>
                    <p className="text-center text-white text-xl mt-6 font-semibold">
                      🎉 That's the power of paying just a little bit more!
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className="bg-white/10 backdrop-blur-xl rounded-3xl shadow-2xl p-8 md:p-10 border border-white/20">
              ...
              <div className="grid md:grid-cols-2 gap-6">
                {results.scenarios.map((scenario, index) => (
                  <div 
                    key={index} 
                    className={`bg-gradient-to-br ${scenario.gradient} rounded-3xl p-8 shadow-2xl border-2 border-white/30 relative overflow-hidden group hover:scale-[1.02] transition-transform`}
                  >
                    ...
                    <div className="relative z-10">
                      <div className="flex items-start justify-between mb-4">
                        ...
                        {index === 0 && (
                          <span className="bg-red-200 text-red-900 text-sm font-bold px-4 py-2 rounded-full">
                            ⚠️ TRAP
                          </span>
                        )}
                        {index === 3 && (
                          <span className="bg-white text-green-700 text-sm font-bold px-4 py-2 rounded-full">
                            ⭐ BEST
                          </span>
                        )}
                      </div>
                      ...
                      {index > 0 && (
                        <div className="bg-white/20 backdrop-blur rounded-xl p-4 border border-white/30">
                          <p className="text-white font-bold text-xl mb-1">
                            💰 Save {formatCurrency(results.scenarios[0].totalInterest - scenario.totalInterest)}
                          </p>
                          <p className="text-white text-base">
                            Get free {formatTime(results.scenarios[0].months - scenario.months)} sooner!
                          </p>
                        </div>
                      )}
                    </div>
            ...
            <div className="bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-600 rounded-3xl shadow-2xl p-8 md:p-12 border-2 border-white/30">
              <div className="text-center mb-10">
                ...
                <p className="text-2xl text-white/95 mb-3">
                  Real strategies that real people use. Total potential: <span className="font-black text-yellow-300 text-3xl">{formatCurrency(results.moneySavingTips.reduce((sum, t) => sum + t.amount, 0))}/month!</span>
                </p>
                      ...
                      <div className="bg-white/10 backdrop-blur rounded-xl p-4 border border-white/20 mb-3">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <div>
                            ...
                            <p className="text-white text-lg font-bold">{formatCurrency(savings)}</p>
                          </div>
                          <div>
            ...
            <div className="bg-gradient-to-br from-green-500 to-emerald-600 rounded-3xl shadow-2xl p-8 md:p-12 text-center border-2 border-white/30">
              ...
              <h3 className="text-4xl md:text-5xl font-black text-white mb-4">Ready to Take Control?</h3>
              <p className="text-2xl text-white/95 mb-8 max-w-3xl mx-auto">
                Download your personalized plan or share this eye-opening tool with friends and family.
              </p>
              <div className="flex flex-col md:flex-row gap-4 justify-center">
                  <button
                      onClick={generatePDF}
                      className="inline-flex items-center justify-center gap-3 bg-white text-green-600 hover:bg-gray-100 font-bold px-10 py-5 rounded-2xl transition-all transform hover:scale-[1.05] shadow-2xl text-xl"
                  >
                      <Download className="w-8 h-8" />
                      Download My Freedom Plan
                  </button>
                  <button
                      onClick={() => setShowShareModal(true)}
                      className="inline-flex items-center justify-center gap-3 bg-blue-500 text-white hover:bg-blue-600 font-bold px-10 py-5 rounded-2xl transition-all transform hover:scale-[1.05] shadow-2xl text-xl"
                  >
            ...
            <div className="text-center py-8">
              <p className="text-gray-400 text-lg mb-2">
                Made with ❤️ by{' '}
                <a 
                  href="https://secretsantamatch.com" 
                  className="text-purple-400 hover:text-purple-300 font-bold underline decoration-2 decoration-purple-400/50 hover:decoration-purple-300/50 transition"
                  target="_blank"
              ...
              <p className="text-gray-500 text-base">
                Free Secret Santa generator • Beautiful cards • No email required
              </p>
            </div>
          </div>
        )}
            ...
            <div className="p-8 text-white space-y-6">
              <div className="bg-white/10 backdrop-blur rounded-2xl p-6 border border-white/20">
                ...
                <p className="text-white/90 leading-relaxed text-lg">
                  Credit card companies design minimum payments to maximize their profit. At typical 2% minimums, 
                  they ensure you stay in debt for decades while they collect interest. It's completely legal, 
                  but it's designed to work against you. The calculator shows you the real numbers they hope you never calculate.
                </p>
              </div>
              ...
              <div className="bg-white/10 backdrop-blur rounded-2xl p-6 border border-white/20">
                ...
                <p className="text-white/90 leading-relaxed mb-3 text-lg">
                  Attack your highest interest rate debt first while paying minimums on everything else. 
                  This mathematically saves you the most money.
                </p>
                <ul className="space-y-2 text-white/80 text-lg">
                  <li className="flex items-start gap-2">
                    <CheckCircle className="w-6 h-6 text-green-400 flex-shrink-0 mt-0.5" />
                    <span>List all debts from highest to lowest interest rate</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <CheckCircle className="w-6 h-6 text-green-400 flex-shrink-0 mt-0.5" />
                    <span>Pay minimums on everything except the highest rate</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <CheckCircle className="w-6 h-6 text-green-400 flex-shrink-0 mt-0.5" />
                    <span>Throw all extra money at the highest rate debt</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <CheckCircle className="w-6 h-6 text-green-400 flex-shrink-0 mt-0.5" />
                    <span>When paid off, roll that payment to the next highest</span>
                  </li>
                </ul>
              </div>
              ...
              <div className="bg-white/10 backdrop-blur rounded-2xl p-6 border border-white/20">
                ...
                <p className="text-white/90 leading-relaxed text-lg">
                  Pay off your smallest balance first for quick wins. Not mathematically optimal, but the motivation 
                  from seeing debts disappear keeps people going. Sometimes psychology beats math!
                </p>
              </div>
              ...
              <div className="bg-white/10 backdrop-blur rounded-2xl p-6 border border-white/20">
                ...
                <p className="text-white/90 leading-relaxed text-lg">
                  Move high-interest debt to a 0% APR promotional card. You get 12-18 months to pay down the balance 
                  without new interest piling up. Just don't add new debt and pay it off before the promo ends!
                </p>
              </div>
                ...
                <div className="space-y-4">
                  <div className="bg-white/5 rounded-xl p-4">
                    ...
                    <p className="text-white/80 text-base">
                      "I used the coffee savings tip and meal prepping. Added $200/month extra to my Chase card. 
                      Now I'm debt-free and saving for a down payment. This calculator was my wake-up call!"
                    </p>
                  </div>
                  <div className="bg-white/5 rounded-xl p-4">
                    <p className="font-bold mb-2 text-lg">Mike, 28 - Eliminated $8,000 in 10 months</p>
                    <p className="text-white/80 text-base">
                      "Started a side hustle walking dogs on Rover. Combined with cutting subscriptions, I was 
                      paying an extra $300/month. Didn't realize how much years of minimum payments would cost me."
                    </p>
                  </div>
                </div>
              </div>
